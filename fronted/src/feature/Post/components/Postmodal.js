...
import { createPostAsync, fetchAllPostsByUserAsync, selectPostUploadStatus } from '../Postslice';
...
const Postmodal = ({ setOpenCretePostmodal }) => {
  ...
  const PostUploadStatus = useSelector(selectPostUploadStatus);
  ...
  const handlePost = () => {
    if(Caption!=="" && postImageUrl!=="") {
        dispatch(createPostAsync({
          PostType:"image",
          PostPath: postImageUrl,
          TaggedUser:[],
          TotalLikes:0,
          Caption
        }));
  ...
  useEffect(()=>{
    // console.log({IssendPost , PostUploadStatus});
    // if(IssendPost===true && PostUploadStatus==="idle"){
    //   toast.success("Post successfully uploaded", gettoastOptions());
    //   setIsSendPost(false);
    // }
    // if(PostUploadStatus==="loading"){
    //   toast.info("Uploading Post.....", gettoastOptions());
    //   // setIsSendPost(true);
    // }
  },[IssendPost, PostUploadStatus])
    ...
    <div className='absolute inset-0 shadow-2xl bg-opacity-40 backdrop-blur-sm'>
      <div className='flex justify-center items-center h-screen'>
        <div className='grid grid-cols-2  bg-white rounded-lg lg:w-1/2 lg:h-1/2 sm:w-3/4  border-2'>
          {postImageUrl != "" ? 
            <div className='flex'>
              <img src={postImageUrl} className=' p-2  w-full' alt="" />
            </div> :
